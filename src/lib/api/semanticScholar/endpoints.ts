...
import { logPapers } from "../../../utils/savepapers.js";
...
//     );
//     logPapers("searchPapers", finalParams.query, response.data.data).catch(console.error);
...
export const searchPapers = async (
  ...
  /* ── 0. Build finalParams ─────────────────────────────── */
  const finalParams =
    params instanceof FilterBuilder ? params.build() : params;

  if (finalParams.query) {
    finalParams.query = sanitizeQuery(finalParams.query);
  }

  /* ── 1. Ensure we always request “abstract” ───────────── */
  if (!finalParams.fields) finalParams.fields = "";
  if (Array.isArray(finalParams.fields)) {
    finalParams.fields = finalParams.fields.join(",");
  }

  const parts = finalParams.fields
    .split(",")
    .map(s => s.trim())
    .filter(Boolean);

  if (!parts.includes("abstract")) parts.push("abstract");
  ...
  /* ── 2. Paging vars ───────────────────────────────────── */
  const limit  = Math.min(finalParams.limit ?? 100, 100);
  let   offset = finalParams.offset ?? 0;
  ...
  while (offset + limit < 1000) {
    const qs = buildQueryParams({ ...finalParams, limit, offset });

    /* --- fetch one page --- */
    let page;
    try {
      page = await semanticScholarClient
        .get(`/paper/search?${qs}`)
        .then(res => res.data);
    } catch (error: unknown) {
      if (axios.isAxiosError(error)) {
        if (error.response?.status === 429)
          throw new Error("Rate limit exceeded (1 rps). Try later.");
        if (error.response?.status === 400)
          throw new Error(`API Error: ${error.response.data.error ?? "Bad parameters"}`);
        if (error.response?.status === 404) throw new Error("Resource not found");
        if (error.response?.status === 500) throw new Error("S2 server error");
      }
      throw error;
    }

    /* --- deduplicate --- */
    const fresh = (page.data as Paper[]).filter((p: Paper) => {
      if (!p.paperId || seenIds.has(p.paperId)) return false;
      seenIds.add(p.paperId);
      return true;
    });

    /* --- 🔄 batch-fetch missing abstracts --- */
    const missingIds = fresh
      .filter((p: Paper) => p.abstract == null && p.paperId)
      .map(p => p.paperId as string);

    if (missingIds.length) {
      try {
        const detailed = await getPapersBatch({
          ids: missingIds,
          fields: "abstract",
        });

        const byId = new Map(detailed.map(p => [p.paperId, p.abstract]));
        fresh.forEach((p: Paper) => {
          if (p.abstract == null) p.abstract = byId.get(p.paperId!) ?? undefined;
        });
      } catch (err: unknown) {
        const msg = err instanceof Error ? err.message : String(err);
        console.warn("Batch fallback failed:", msg);
      }
    }

    /* --- write to CSV --- */
    csvPath = await logPapers(
      "searchPapers",
      finalParams.query,
      fresh,
      { append: !first, filePath: csvPath }
    );
    first = false;
  ...
  return { total: ALL.length, offset: 0, data: ALL, next: undefined };
};
